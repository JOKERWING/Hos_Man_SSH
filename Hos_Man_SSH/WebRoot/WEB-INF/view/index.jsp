...
<script>
	$(function() {
		$.datepicker._gotoToday = function() {
			$("#indate").datepicker('setDate', new Date()).datepicker('hide')
					.blur();
		};
...
<script>
						...
						function(data) {
							...
							$
									.each(
											lst,
											function(i, n) {
												str += "<div class='grid1_of_3'><div class='grid1_of_3_img'><a href='HosMan/rooms?type="+n.h_name+"'  id='tfj1'> <img src='Picture/"+n.hi_type+".jpg' alt='' /></a></div><h4><a class='' href='HosMan/rooms?type="
														+ n.h_name
														+ "'  style='float: left;cursor:point;'>"
														+ n.h_name
														+ "</a><a class='a1 '   style='float: left;text-transform:none; text-decoration:none;'>"
														+ n.hi_price
	...
	function book() {
		...
		window.location = "HosMan/book?type=" + type + "&&indate=" + indate
				+ "&&outdate=" + outdate + "&&pnum=" + pnum;
	}
</script>
...
<script>
	...
	function dateset() {
		...
		if (indate == "年-月-日") {
			...
			indate1 = mydate.getFullYear() + "-" + parseInt(mydate.getMonth()+1) + "-"
					+ mydate.getDate();
			var nextdate = new Date(mydate.getTime() + 24 * 60 * 60 * 1000);
			//outdate1 = nextdate.toLocaleDateString();
			outdate1 = nextdate.getFullYear() + "-" + parseInt(nextdate.getMonth()+1) + "-"
					+ nextdate.getDate();
		} else {
			indate1 = indate;
			outdate1 = outdate;
